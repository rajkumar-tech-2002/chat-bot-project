...
const saveAudio = async (req, res) => {
    try {
        const { question_text, answer_text, visitor_id, session_id } = req.body;
        const audio_path = req.file ? req.file.path : null;

        await pool.execute(
            `INSERT INTO voice_logs (visitor_id, session_id, question_text, answer_text, audio_path)
             VALUES (?, ?, ?, ?, ?)`,
            [visitor_id || null, session_id || null, question_text || '', answer_text || '', audio_path || '']
        );
