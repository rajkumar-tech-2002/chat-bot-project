...
exports.endSession = async (req, res) => {
  ...
  try {
    ...
    console.log(`Ending session for visitor: ${visitor.name} (${visitor.email})`);

    const history = await Conversation.findByVisitor(visitor_id);
    const audioLogs = await VoiceLog.findByVisitor(visitor_id);
    ...
    const pdfBuffer = await transcriptService.generatePDF(visitor, chatLog);
    const wordBuffer = await transcriptService.generateWord(visitor, chatLog);

    let attachments = [
      { filename: 'Conversation_Transcript.pdf', content: pdfBuffer },
