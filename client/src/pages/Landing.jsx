...
const Landing = () => {
    ...
    return (
        <div className="flex-1 pt-36 overflow-y-auto overflow-x-hidden">
                    ...
                    <div className="flex flex-col sm:flex-row justify-center gap-6">
                        ...
                        <Link to="/login">
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                className="px-10 py-5 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-white/10 rounded-2xl font-black flex items-center justify-center space-x-3 shadow-xl transition-all"
                            >
                                <ShieldCheck size={22} />
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 border-t border-slate-100 dark:border-white/5 transition-colors duration-500">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-left">
                    {features.map((f, i) => (
                        <motion.div
                            ...
                            className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-3xl border border-white dark:border-white/10 p-10 rounded-[2.5rem] shadow-xl shadow-blue-900/5 hover:shadow-2xl hover:shadow-blue-500/10 transition-all group"
                        >
                            <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl w-fit mb-8 shadow-inner group-hover:scale-110 group-hover:bg-blue-50 dark:group-hover:bg-blue-900/30 transition-all duration-500">
                                {f.icon}
                            </div>
                            <h3 className="text-2xl font-black mb-4 text-slate-900 dark:text-white tracking-tight">{f.title}</h3>
                            <p className="text-slate-500 dark:text-slate-400 font-medium group-hover:text-slate-700 dark:group-hover:text-slate-200 transition-colors leading-relaxed">
                                {f.desc}
                            </p>
                        </motion.div>
                    ))}
            ...
            <section className="bg-white/40 dark:bg-slate-900/20 border-y border-slate-100 dark:border-white/5 py-24 mb-20 backdrop-blur-sm transition-colors duration-500">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-2 md:grid-cols-4 gap-12 text-center">
                    ...
                    <div>
                        <div className="text-5xl font-black text-pink-600 dark:text-pink-500 mb-3 tracking-tighter">15+</div>
                        <div className="text-slate-400 dark:text-slate-500 uppercase text-[10px] tracking-[0.3em] font-black">Campus Hostels</div>
                    </div>
                </div>
            </section>
